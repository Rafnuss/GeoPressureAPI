"""
GeoPressure API - Map Cloud Function

This module provides a Google Cloud Function endpoint for generating probability maps
for geolocator analysis using Mean Squared Error (MSE) calculations between observed
pressure data and ERA5 atmospheric reanalysis.
...
with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as tmp_file:
    ...
    tmp_file.flush()
    # Initialize the GeoPressure map processor
    process = GP_map_v2(gee_api_address, tmp_file.name, highVolumeEndpoint)


@functions_framework.http
def map(request):
    """
    Google Cloud Function entry point for map generation API.

    Handles HTTP requests for MSE-based probability map generation using
    geolocator pressure measurements and ERA5 atmospheric data.
    ...
    Request Parameters (JSON):
        ...
        - label (array): Array of labels for grouping measurements

        Optional:
        - scale (float): DEM resolution scale factor (default: 10)
        - maxSample (int): Maximum samples per label (default: 250)
        - margin (float): Altitude tolerance in meters (default: 30)
        - includeMask (bool): Include altitude probability mask (default: True)
    ...
    if request.method == "POST":
        ...
        if request_json:
            try:
                ...
                print(f"Catastrophic error in map processing: {e}")
                import traceback
